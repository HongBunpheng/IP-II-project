<template>
    <div class="city-slider">
        ...
        <!-- Animated background -->
        <div v-if="animating" class="background background-next" :style="{ backgroundImage: `url(${nextBackground})` }"
        ...
        <div class="container">
            <div class="slide">
                <div v-for="(item, i) in visibleItems.slice(1, 6)" :key="i" class="item" :style="getStyle(i)"
                    @click="goToSlide((index + i + 1) % allItems.length)"></div>
            </div>
        </div>
    </div>
</template>
